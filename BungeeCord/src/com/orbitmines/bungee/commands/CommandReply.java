...
public class CommandReply extends Command {
    ...
    private final String[] alias = {"/reply","/r"};
    ...
    public void dispatch(BungeePlayer omp, String[] a) {
        ...
        if (a.length >= 2) {
            if (omp.getLastMsg() != null) {
                ProxiedPlayer p2 = ProxyServer.getInstance().getPlayer(omp.getLastMsg());
                ...
                omp.sendMessage(new Message("§9§lMSG §8» §7Je hebt niemand om op te reageren.", "§9§lMSG §8» §7You have nobody to reply to."));
            }
        } else {
            p.sendMessage(new TextComponent("§9§lMSG §8» §7" + omp.getMessage(new Message("Gebruik", "Use")) + " §9" + a[0].toLowerCase() + " <message>§7."));
        }
    }
}
